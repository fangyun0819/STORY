  ...
  _renderMembers(){
    return this.state.members.map((item, i) => {
      ...
      if( i !== 0){
        return (
          <div>
            { this.state.confirmedMembers[i-1] ? <a>已加入</a> : "未加入"}
            <Avatar alt="members" src={this.state.confirmedMembers[i-1].image} />
            { this.state.confirmedMembers[i-1].confirmed ? <a>{this.state.confirmedMembers[i-1].name}</a> : this.state.confirmedMembers[i-1].name}
            <br/>
          </div>
        )
      }
